...
for filepath in $CLUSTER_CLIENT_INFO*; do
  ...
  echo "file:$FILE"
  python3 weights_optimization.py DeNASA --tor_users_to_location $LOCATION_TO_USERS --obj_function $OBJ_FUNC --theta $THETA --pmatrix $PMATRIX --penalty_vanilla $VANILLA_EPL --client_clust_representative $CLUSTER_CLIENT_INFO$FILE --cluster_file $CLUSTER_GUARD_FILE --out_dir $OUT --network_state $NETWORK_STATE 2>&1
