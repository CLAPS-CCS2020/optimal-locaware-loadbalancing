...
for filepath in $CLUSTER_CLIENT_INFO*; do
  ...
  python3 weights_optimization.py DeNASA --tor_users_to_location $LOCATION_TO_USERS --obj_function $OBJ_FUNC --theta $THETA --pmatrix $PMATRIX --penalty_vanilla $VANILLA_EPL_DENASA_G --client_clust_representative $CLUSTER_CLIENT_INFO$FILE --cluster_file $CLUSTER_GUARD_FILE --out_dir $OUT --network_state $NETWORK_STATE 2>&1
  # run the solver
  $CLP_PATH $PWD/$OUT/location_aware_with_obj_$OBJ_FUNC\_theta_$THETA.mps
  -minimize -dualsimplex -solution $PWD/$OUT/denasa_g__obj$OBJ_FUNC\_theta_$THETA\_$FILE.sol 2>&1
  # run deNasa g&e with the solver's output
  python3 weights_optimization.py DeNASA_EXIT --tor_users_to_location $LOCATION_TO_USERS --obj_function $OBJ_FUNC --theta $THETA --pmatrix $PMATRIX --penalty_vanilla $VANILLA_EPL_DENASA_GE --client_clust_representative $CLUSTER_CLIENT_INFO$FILE --cluster_file $CLUSTER_GUARD_FILE --out_dir $OUT --network_state $NETWORK_STATE --deNasa_so_guards $PWD/$OUT/denasa_g_obj$OBJ_FUNC\_theta_$THETA\_$FILE.sol --fp_to_asn $FP_TO_ASN 2>&1
